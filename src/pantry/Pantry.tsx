...
} from "../constants/predefinedIngredients";
...
const App: React.FC = () => {
  ...
  const navigation =
    useNavigation<NativeStackNavigationProp<PantryStackParamList>>();
  ...
  const filteredPredefined = useMemo(() => {
    ...
    (
      Object.keys(
        PREDEFINED_INGREDIENTS
      ) as (typeof CATEGORY_INFOS)[number]["key"][]
    ).forEach((cat) => {
      if (!searchTerm) {
  ...
  const generateRecipe = async () => {
    ...
    if (cleanedIngredients.length === 0) {
      Alert.alert(
        MESSAGES.PANTRY_LABEL_ERROR,
        MESSAGES.PANTRY_LABEL_ERROR_DEFAULT
      );
      setIsLoading(false);
      return;
    }
    ...
    if (cleanedIngredients.length > 10) {
      Alert.alert(
        MESSAGES.PANTRY_LABEL_ERROR_401,
        MESSAGES.PANTRY_LABEL_ERROR_DEFAULT_401
      );
      setIsLoading(false);
      return;
    }
                    ...
                    >
                      <Text
                        style={[
                          ...
                          ingredients.includes(ingredient) &&
                            styles.ingredientTagTextSelected,
                        ]}
                      >
                        {ingredient}
                      </Text>
